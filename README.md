...
- **参数解析**：自动区分 `@PathVariable`、`@RequestParam`、`@RequestBody`、`@RequestHeader`、`@CookieValue`，标注是否必填及默认值
- **泛型支持**：基于 `ResolvableType` 正确解析 `ResponseEntity<User>`、`List<Order>` 等嵌套泛型返回类型
- **框架过滤**：自动跳过 `BasicErrorController` 等 Spring 内置接口
...
- **性能优化**：使用 `ConcurrentHashMap` 按类缓存解析结果，避免重复 I/O
- **优雅降级**：源文件不存在时（如 JAR 部署）自动回退为方法名

### 阶段三（OpenAPI + HTML 预览）

- **OpenAPI 3.0 JSON**：生成标准 OpenAPI 规范文件，可导入 Postman、Apifox 等工具
- **交互式 HTML 预览**：生成完全自包含的 HTML 文件（内嵌 OpenAPI JSON），双击即可在浏览器中打开，无需 HTTP 服务器
- **多格式输出**：通过配置自由选择输出 Markdown / OpenAPI / HTML
- **可配置输出目录**：所有文档统一输出到 `outdoc/` 目录
...
应用启动后，在项目的 `outdoc/` 目录下会自动生成三种格式的文档：

```
✅ DocLeaf 文档已生成：/path/to/outdoc/API_DOCUMENTATION.md
✅ DocLeaf 文档已生成：/path/to/outdoc/openapi.json
✅ DocLeaf 文档已生成：/path/to/outdoc/docleaf-index.html
```

### 查看 HTML 预览

HTML 文件已内嵌完整的 OpenAPI JSON 数据，**双击 `outdoc/docleaf-index.html` 即可直接在浏览器中打开**，无需任何 HTTP 服务器或额外端口。
...
# 禁用 JavaDoc 提取（JAR 部署场景）
docleaf.javadoc.enabled=false
```
...
├── pom.xml
├── README.md
├── outdoc/                              # 文档输出目录（自动生成，已 gitignore）
│   ├── API_DOCUMENTATION.md             #   Markdown 格式文档
│   ├── openapi.json                     #   OpenAPI 3.0 JSON
│   └── docleaf-index.html               #   HTML 预览页面（自包含，双击可开）
└── src/main/
    ├── java/com/docleaf/
    │   ├── DocLeafApplication.java      # 主启动类
    │   ├── DocLeafDocGenerator.java     # 核心文档生成器（集成三种输出）
    │   ├── config/
    │   │   └── DocLeafProperties.java   # 配置属性类（@ConfigurationProperties）
    │   ├── model/
    │   │   ├── ApiInfo.java             # API 接口信息模型
    │   │   └── ApiParamInfo.java        # API 参数信息模型
    │   ├── javadoc/
    │   │   ├── JavaDocExtractor.java    # JavaDoc 注释提取器（正则 + 缓存）
    │   │   ├── ClassJavaDoc.java        # 类级 JavaDoc 数据模型
    │   │   └── MethodJavaDoc.java       # 方法级 JavaDoc 数据模型
    │   ├── generator/
    │   │   ├── OpenApiGenerator.java    # OpenAPI 3.0 JSON 生成器
    │   │   └── HtmlPreviewGenerator.java# HTML 预览页面生成器（内嵌 JSON）
    │   └── demo/                        # 示例 Controller（已 gitignore）
    │       ├── UserController.java
...
2. **自包含**：OpenAPI JSON 内嵌为 JS 变量，单个 HTML 文件即可完整展示，无需 HTTP 服务器
3. **极简**：约 200 行代码，易于理解和定制

### JavaDoc 提取：正则 + 行扫描

选择正则而非 JavaParser 等 AST 库，理由如下：

1. **零额外依赖**：不引入第三方解析库
2. **足够用**：JavaDoc 格式相对固定，正则 + 行扫描已覆盖绝大多数场景
3. **高性能**：配合 `ConcurrentHashMap` 缓存，避免重复文件 I/O
